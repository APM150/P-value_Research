...
class PCA_pvalue:
    ...
    def all_p_values(self, Xtest, distribution=None):
        ''' taking the min of all component (suggested by prof. Hayes) '''

        if distribution == None:
            distribution = self.gaussian_pvalue

        return np.min(self.pvalue(Xtest, distribution=distribution), axis=2) * 2
    ...
    def predict(self, Xtest, distribution=None):
        ...
        return np.array(np.argmax(self.all_p_values(Xtest, distribution=distribution), axis=1), dtype='int16')
